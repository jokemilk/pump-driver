...
static struct PUMP_STATE pump_state_temp;
...
void default_global_values(struct pump_dev *dev)
{
	...
	pump_state_temp = dev->pump_state;
}
	...
	if (cnt++ == 25)
	{
		...
		if (pump_device->pump_state.direction == 0)
		{
			...
			if (pump_device->pump_state.step == 0)
			{
				...
				pump_device->pump_state.timer_reload =
						pump_device->pump_state.time_push_acc;
			}
			...
			else if (pump_device->pump_state.step == 4)
			{
				...
				pump_device->pump_state.timer_reload =
						pump_device->pump_state.time_push_avg;
			}
		...
		else
		{
			...
			if (pump_device->pump_state.step == 0)
			{
				...
				pump_device->pump_state.timer_reload =
						pump_device->pump_state.time_pull_acc;
			}
			else if ((pump_device->pump_state.step > 0)
					&& (pump_device->pump_state.step < 4))
			{
			...
			else if (pump_device->pump_state.step == 4)
			{
				...
				pump_device->pump_state.timer_reload =
						pump_device->pump_state.time_pull_avg;
			}
			else if (pump_device->pump_state.step <= 8)
			{
				freq_1 = pump_device->pump_state.pwm_rate[1]
...
//		Auto_zero();
		printk("stop timer_0\n");
		break;
	case PUMP_SET_PUSH_ACC:
	{
		if (!capable(CAP_SYS_ADMIN))
			return -EPERM;
		retval = __get_user(pump_state_temp.time_push_acc, (int __user *) arg);
	}
		break;
	case PUMP_SET_PUSH_AVG:
	{
		if (!capable(CAP_SYS_ADMIN))
			return -EPERM;
		retval = __get_user(pump_state_temp.time_push_avg, (int __user *) arg);
	}
		break;
	case PUMP_SET_PULL_ACC:
	{
		if (!capable(CAP_SYS_ADMIN))
			return -EPERM;
		retval = __get_user(pump_state_temp.time_pull_acc, (int __user *) arg);
	}
		break;
	case PUMP_SET_PULL_AVG:
	{
		if (!capable(CAP_SYS_ADMIN))
			return -EPERM;
		retval = __get_user(pump_state_temp.time_pull_avg, (int __user *) arg);
	}
		break;
	case PUMP_SET_PWM0:
	{
		if (!capable(CAP_SYS_ADMIN))
			return -EPERM;
		retval = __get_user(pump_state_temp.pwm_rate[0], (int __user *) arg);
	}
		break;
	case PUMP_SET_PWM1:
	{
		if (!capable(CAP_SYS_ADMIN))
			return -EPERM;
		retval = __get_user(pump_state_temp.pwm_rate[1], (int __user *) arg);
	}
		break;
